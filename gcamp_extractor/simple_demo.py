...
parser.add_argument("--neuron", nargs='?', default="all")
...
with napari.gui_qt():
    ...
    if args.neuron != "all":
        viewer.add_image( (gmm_ellipsoid > 0).astype(int) * (gmm_ellipsoid <= s).astype(int), name=neuron_name + " ellipsoid", blending='additive', colormap="cyan", opacity=0.7, visible=True)
        viewer.add_image(neuron_pixels, name=neuron_name, blending='additive', colormap="magenta", opacity=0.7, visible=True)
